...
exports.requestPasswordReset = (req, res) => {
  ...
  Account.getAccountByEmail(email, (err, result) => {
    ...
    const resetToken = crypto.randomBytes(32).toString('hex'); // Create a unique token for password reset
    const resetTokenExpiry = Date.now() + 3600000; // Token expires in 1 hour

    // Save the reset token and its expiry date in the database (assuming you have a resetToken and resetTokenExpiry columns)
    Account.saveResetToken(account.user_id, resetToken, resetTokenExpiry, (err) => {
      if (err) {
        console.error('Error saving reset token:', err);  // Log the full error
...
exports.resetPassword = (req, res) => {
  ...
  Account.getAccountByResetToken(token, (err, result) => {
    if (err || !result || result.length === 0) {
    ...
    if (Date.now() > account.resetTokenExpiry) {
    ...
    bcrypt.hash(newPassword, 10, (err, hashedPassword) => {
      ...
      Account.updatePassword(account.user_id, hashedPassword, (err) => {
        if (err) return res.status(500).send('Error updating password.');

        // Clear the reset token from the database
        Account.clearResetToken(account.user_id, (err) => {
          if (err) return res.status(500).send('Error clearing reset token.');
