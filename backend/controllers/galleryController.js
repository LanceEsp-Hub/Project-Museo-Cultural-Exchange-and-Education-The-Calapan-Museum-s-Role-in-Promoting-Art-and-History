...
exports.updateGalleryItem = async (req, res) => {
  ...
  let imagePath = req.body.imageUrl || null; // Default to existing image

  console.log('Incoming Request Data:', req.body);  // Log the form fields
...
exports.archiveGalleryItem = async (req, res) => {
  ...
  try {
    const result = await Gallery.archiveGalleryItem(id);

    if (result && result.affectedRows > 0) {
      return res.status(200).json({ message: 'Gallery item archived successfully.' });
    } else {
      return res.status(404).json({ message: 'Gallery item not found.' });
    }
  } catch (error) {
    console.error('Error archiving gallery item:', error);
    return res.status(500).json({ message: 'Failed to archive gallery item.' });
  }
};

exports.reactivateGalleryItem = async (req, res) => {
  const { id } = req.params;

  try {
    const result = await Gallery.reactivateGalleryItem(id);

    if (result && result.affectedRows > 0) {
      return res.status(200).json({ message: 'Gallery item reactivated successfully.' });
    } else {
      return res.status(404).json({ message: 'Gallery item not found.' });
    }
  } catch (error) {
    console.error('Error reactivating gallery item:', error);
    return res.status(500).json({ message: 'Failed to reactivate gallery item.' });
  }
};