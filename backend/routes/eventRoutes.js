...
router.post('/', upload.single('events'), eventController.addEventItem);  // Use upload.single('image') for file upload

// Route for getting a specific event by ID
router.get('/:id', eventController.getEventById);

// Route for updating an event by ID with image upload
router.put('/:id', upload.single('events'), eventController.updateEvent);  // Use upload.single('image') for file upload
